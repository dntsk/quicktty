...
grep -F -x '        CURRENT_PROJECT_VERSION: 12' "$project_spec" >/dev/null \
    || fail 'project spec does not set CURRENT_PROJECT_VERSION to 12'
...
assert_equals "$RELEASE_LABEL_DEFAULT" 0.1.3.beta-3
assert_equals "$RELEASE_ARCHIVE_NAME" QuickTTY.xcarchive
assert_equals "$RELEASE_DMG_NAME" QuickTTY-0.1.3.beta-3-arm64.dmg
assert_equals "$RELEASE_STAGE_NAME" QuickTTY-0.1.3.beta-3-stage
...
assert_equals "$(release_appcast_download_url_prefix)" \
    https://github.com/dntsk/quicktty/releases/download/v0.1.3.beta-3/
