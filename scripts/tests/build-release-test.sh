...
grep -F -x '        CURRENT_PROJECT_VERSION: 15' "$project_spec" >/dev/null \
    || fail 'project spec does not set CURRENT_PROJECT_VERSION to 15'
...
grep -F -x '        MARKETING_VERSION: 0.1.4' "$project_spec" >/dev/null \
    || fail 'project spec does not set MARKETING_VERSION to 0.1.4'
...
assert_equals "$RELEASE_LABEL_DEFAULT" 0.1.4
assert_equals "$RELEASE_ARCHIVE_NAME" QuickTTY.xcarchive
assert_equals "$RELEASE_DMG_NAME" QuickTTY-0.1.4-arm64.dmg
assert_equals "$RELEASE_STAGE_NAME" QuickTTY-0.1.4-stage
...
assert_equals "$(release_appcast_download_url_prefix)" \
    https://github.com/dntsk/quicktty/releases/download/v0.1.4/
