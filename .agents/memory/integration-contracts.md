...
`GhosttyBridge` — единственная точка взаимодействия Swift-кода QuickTTY с нестабильным upstream C API Ghostty. Остальная часть приложения зависит от устойчивых Swift-типов и идентификаторов проекта, а не от заголовков, символов, структур, callback-типов или правил владения Ghostty.
...
- На границе используются собственные Swift value types, идентификаторы и ошибки QuickTTY.
...
- Production bootstrap с `configURL: nil` намеренно создаёт только встроенные finalized defaults через `ghostty_config_new` → `ghostty_config_finalize`: default Ghostty user files и recursive loader не вызываются. Explicit URL загружает только указанный root через `ghostty_config_load_file`, затем сохраняет его recursive includes и выполняет finalize. В закреплённом source `config-file` по умолчанию пуст (`Vendor/ghostty/src/config/Config.zig:2452`), а recursive loader при пустом списке является no-op (`Vendor/ghostty/src/config/Config.zig:4171-4173`), но nil-path всё равно его не вызывает для явного built-in-only контракта. `ConfigController` читает и наблюдает пользовательский source `~/.config/quicktty/config`, разбирает и отфильтровывает принадлежащие QuickTTY assignments с префиксом `quicktty-`, а оставшуюся Ghostty-конфигурацию атомарно записывает в `~/.config/quicktty/.ghostty-effective-config`. Этот сгенерированный effective file передаётся в reload `GhosttyBridge`; пользователь его не редактирует. Parsed diagnostics QuickTTY assignments не блокируют успешное применение и публикуются после успешного reload. При ошибке reload controller восстанавливает предыдущие active config/document и effective bytes (либо удаляет effective file, если его не было); последняя валидная конфигурация bridge остаётся активной. Legacy migration отсутствует.
...
- macOS `nsview` передаётся как unretained platform pointer (`QuickTTY/Integration/GhosttyBridge/GhosttySurfaceView.swift:353-357`; pinned runtime принимает этот pointer в `Vendor/ghostty/src/apprt/embedded.zig:347-379`). Bridge удерживает `GhosttySurfaceView` на всём активном lifecycle, а `close()` выполняет `ghostty_surface_free` и thread joins, пока вызванный метод всё ещё удерживает view живым (`QuickTTY/Integration/GhosttyBridge/GhosttySurfaceView.swift:92-105`). Поэтому renderer не переживает unretained `NSView`.
...
- `QuickTTYApplication.main` всегда выполняет глобальный `ghostty_init` до создания `NSApplication`, включая hosted unit tests.
...
- `performKeyEquivalent` работает только для focused keyDown. Он синхронно сопоставляет event с активной `ShortcutConfiguration`: structural/application action возвращается AppKit, terminal action выполняется через typed allowlist, performable `false` возвращает событие в normal terminal path, unmatched event не поглощается. Production dispatch не вызывает `ghostty_surface_key_is_binding`; из upstream сохранены Ctrl+Return, точный Ctrl+/ и timestamp-based `doCommand(by:)` redispatch для command/control. Ghostty menu, key tables и sequence UI не портированы.
...
- Production client изолирован MainActor. Standard использует `NSPasteboard.general`; selection намеренно использует отдельный named pasteboard `com.dntsk.QuickTTY.selection`. Namespace принадлежит QuickTTY, а отдельная-board семантика повторяет закреплённый upstream `com.mitchellh.ghostty.selection` без совместного clipboard между приложениями.
...
## Configurable shortcuts и terminal action boundary

- `ShortcutAction` — закрытый first-party registry со stable config ID, default chord, scope, target, menu и perform policy. `TerminalShortcutAction` является typed allowlist; только его exhaustive mapping может создать фиксированную строку для `ghostty_surface_binding_action`. Произвольная action string не принимается из config и не проходит через публичный bridge API.
- `ConfigController` строит local configuration построчно: валидные инструкции применяются последовательно, invalid-known action сохраняет последнее active значение, удалённая строка возвращает default, а chord получает последний валидный владелец. Global chord разрешается отдельно и имеет приоритет над local owner; неудачная Carbon replacement сохраняет последнюю успешную registration.
- Top-level пользовательские `keybind` молча исключаются из effective bytes. После всех оставшихся assignments и `include` effective config всегда завершается `keybind = clear`, поэтому bindings Ghostty не участвуют в runtime dispatch. Неназначенный event передаётся в `ghostty_surface_key` как terminal input; production-код не вызывает `ghostty_surface_key_is_binding`.
- Active shortcut map принадлежит MainActor `ShortcutController` и синхронно публикуется в `GhosttyBridge`. AppKit menu items обновляются на MainActor. Ни controller, ни config model не хранит `GhosttySurfaceView`, C handle или callback context.
- Shortcut dispatch каждый раз разрешает source `PaneID` через реестр live surfaces на MainActor. Hidden, inactive, detached и закрытая surface не является target. Private C handle используется только синхронно внутри live `GhosttySurfaceView`; после close terminal action возвращает `false` без C-вызова.
- `paste` и `paste-selection` используют существующий target provider broadcast только для panes активной tab. Остальные terminal actions выполняются на focused source pane. Для `copy`, `copy-url`, `clear-screen`, `scroll-to-selection` и `selection-*` core `false` сохраняет normal terminal fallback; consuming actions поглощаются согласно typed policy.
- Read-only, secure-input и mouse-reporting actions не входят в allowlist до появления видимого state, checked menu state и lifecycle cleanup. Interactive Search и URL hover/open остаются обязательными отдельными интеграциями.
...
2. изменить только реализацию и Swift-контракт `GhosttyBridge`, если публичное поведение QuickTTY не меняется;
...
|---|---|---|
| 2026-07-23 | Keyboard shortcut ownership перенесён в typed QuickTTY registry; Ghostty keybind table очищается, terminal actions ограничены allowlist и live MainActor surface lookup, а performable actions сохраняют terminal fallback; после integrated review `make check` прошёл 525 тестов в 27 suites | Configurable shortcuts должны применяться без пересоздания surfaces и без потери обычного input/IME; production `ghostty_surface_key_is_binding` больше не используется |
